		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				if (dfsHelper(neighbour, visited, stackState)) {
					// you've found a backedge in the subcomponent of neighbour
					return true;
				}
				...
				// check if source -> neighbour is a backedge or not

				// check if there exists a path from neighbour to source
