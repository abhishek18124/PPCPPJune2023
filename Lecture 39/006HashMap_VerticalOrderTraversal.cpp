...
void verticalOrder(node* root, map<int, vector<int>>& distMap, int d) {
	...
	// make an entry for the root node in the distance map

	distMap[d].push_back(root->data); // we are saying root->data is present at distance d from the vertical baseline
