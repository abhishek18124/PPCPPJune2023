...
int maxProfit(const vector<int>& weights, const vector<int>& profits,
              int N, int i, int w) {
...
int maxProfitTopDown(const vector<int>& weights, const vector<int>& profits,
...
int maxProfitBottomUp(const vector<int>& weights, const vector<int>& profits, int N, int W) {
	...
	return dp[0][W]; // at the (0, W)th cell of dp[][], we store result of f(0, W)
